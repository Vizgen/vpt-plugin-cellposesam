...
def get_centers(masks, slices):
    ...
    centers = np.array(
        [np.array([centers[i][0] + slices[i][0].start, centers[i][1] + slices[i][1].start]) for i in range(len(slices))]
    )
...
def masks_to_flows_gpu(masks, device=torch.device("cpu"), niter=None):
    ...
    if masks.max() > 0:
        ...
        mu = _extend_centers_gpu(neighbors, meds_p, isneighbor, shape, n_iter=n_iter, device=device)
...
def masks_to_flows_gpu_3d(masks, device=None, niter=None):
    ...
    ext = np.array([[sz.stop - sz.start + 1, sy.stop - sy.start + 1, sx.stop - sx.start + 1] for sz, sy, sx in slices])
    n_iter = 6 * (ext.sum(axis=1)).max() if niter is None else niter

    # run diffusion
    shape = masks_padded.shape
    mu = _extend_centers_gpu(neighbors, centers, isneighbor, shape, n_iter=n_iter, device=device)
    # normalize
    mu /= 1e-60 + (mu**2).sum(axis=0) ** 0.5
...
def labels_to_flows(labels, files=None, device=None, redo_flows=False, niter=None, return_flows=True):
    ...
    if labels[0].shape[0] == 1 or labels[0].ndim < 3 or redo_flows:
        ...
        for n in iterator(nimg):
            ...
            flow = np.concatenate((labels[n], labels[n] > 0.5, vecn), axis=0).astype(np.float32)
...
def flow_error(maski, dP_net, device=None):
    ...
    for i in range(dP_masks.shape[0]):
        flow_errors += mean((dP_masks[i] - dP_net[i] / 5.0) ** 2, maski, index=np.arange(1, maski.max() + 1))
...
def get_masks_torch(pt, inds, shape0, rpad=20, max_size_fraction=0.4):
    ...
    for iter in range(5):
        ...
        seed_masks *= h_slc > 2
    del h_slc
    seeds_new = [tuple((torch.nonzero(seed_masks[k]) + seeds1[k] - 5).T) for k in range(n_seeds)]
    del seed_masks
...
def compute_masks(
    ...
    if (cellprob > cellprob_threshold).sum():  # mask at this point is a cell cluster binary map, not labels
        ...
        p_final = follow_flows(dP * (cellprob > cellprob_threshold) / 5.0, inds=inds, niter=niter, device=device)
        ...
        mask = get_masks_torch(p_final, inds, dP.shape[1:], max_size_fraction=max_size_fraction)
        del p_final
