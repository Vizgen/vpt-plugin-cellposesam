...
class CellposeModel:
            ...
            if do_3D:
                if compute_masks:
                    ...
                    masks = transforms.resize_image(
                        masks, Ly=Ly_0, Lx=Lx_0, no_channels=True, interpolation=cv2.INTER_NEAREST
                    )
                    masks = masks.transpose(1, 0, 2)
                    masks = transforms.resize_image(
                        masks, Ly=Lz_0, Lx=Lx_0, no_channels=True, interpolation=cv2.INTER_NEAREST
                    )
                    masks = masks.transpose(1, 0, 2)
    ...
    def _resize_cellprob(
        self, prob: np.ndarray, to_y_size: int, to_x_size: int, to_z_size: Union[int, None] = None
    ) -> np.ndarray:
        """
        ...
        prob_shape_arr = np.array(prob_shape)
        ...
        if prob.ndim == 2:
            ...
            if squeeze_happened:
                prob = np.expand_dims(
                    prob, int(np.argwhere(prob_shape_arr == 1))
                )  # add back empty axis for compatibility
        elif prob.ndim == 3:
            # 3D case:
            prob = transforms.resize_image(prob, Ly=to_y_size, Lx=to_x_size, no_channels=True)
            ...
            raise ValueError(
                f"gradients have incorrect dimension after squeezing. Should be 2 or 3, prob shape: {prob.shape}"
            )

        return prob

    def _resize_gradients(
        self, grads: np.ndarray, to_y_size: int, to_x_size: int, to_z_size: Union[int, None] = None
    ) -> np.ndarray:
        """
        ...
        grads_shape_arr = np.array(grads_shape)
        ...
        if grads.ndim == 3:
            ...
            if squeeze_happened:
                grads = np.expand_dims(
                    grads, int(np.argwhere(grads_shape_arr == 1))
                )  # add back empty axis for compatibility
        elif grads.ndim == 4:
            ...
            raise ValueError(
                f"gradients have incorrect dimension after squeezing. Should be 3 or 4, grads shape: {grads.shape}"
            )

        return grads

    def _run_net(self, x, augment=False, batch_size=8, tile_overlap=0.1, bsize=256, anisotropy=1.0, do_3D=False):
        ...
        if do_3D:
            ...
            if anisotropy is not None and anisotropy != 1.0:
                ...
                x = transforms.resize_image(x.transpose(1, 0, 2, 3), Ly=int(Lz * anisotropy), Lx=int(Lx)).transpose(
                    1, 0, 2, 3
                )
            yf, styles = run_3D(
                self.net, x, batch_size=batch_size, augment=augment, tile_overlap=tile_overlap, bsize=bsize
            )
            cellprob = yf[..., -1]
