...
class Transformer(nn.Module):
    def __init__(self, backbone="vit_l", ps=8, nout=3, bsize=256, rdrop=0.4, checkpoint=None, dtype=torch.float32):
        ...
        self.W2 = nn.Parameter(
            torch.eye(self.nout * ps**2).reshape(self.nout * ps**2, self.nout, ps, ps), requires_grad=False
        )
    ...
    def forward(self, x):
        ...
        if self.training and self.rdrop > 0:
            ...
            rdrop = (
                torch.rand((len(x), nlay), device=x.device) < torch.linspace(0, self.rdrop, nlay, device=x.device)
            ).to(x.dtype)
...
class CPnetBioImageIO(Transformer):
    ...
    def load_state_dict(self, state_dict):
        ...
        if state_dict["output.2.weight"].shape[0] != self.nout:
            ...
            super().load_state_dict({name: param for name, param in state_dict.items()}, strict=False)
